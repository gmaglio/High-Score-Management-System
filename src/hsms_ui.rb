...
class Main
     ...
     table = Gtk::Table.new(6,    10,    false)
     ...
     sendButton.signal_connect("clicked") do
          ...
          if scoreRec.name.text.match(/[A-Z]{3}/).nil? and scoreRec.score.text.to_i.<0 
             print "regexp does not match\n"
             ...
             dialog.destroy
          elsif scoreRec.name.text.match(/[A-Z]{3}/).nil?
             print "regexp does not match\n"
             dialog.title = "Entry Error"
